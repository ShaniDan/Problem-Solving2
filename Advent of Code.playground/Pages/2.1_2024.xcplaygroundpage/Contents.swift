...
func solution(input: [[Int]]) -> [[Int]] {
    ...
    for row in input {
        ...
        differencesArray.append(differences)
    }
    print("DIFFERENCE: \(differencesArray)")
...
let differences = solution(input: input)

func numberOfSafeReports(input: [[Int]]) -> [[Int]] {
    var result: [[Int]] = []
    
    for row in input {
        
    // get the elements with only negative or positive values
        
        let isOnlyPositive = row.allSatisfy { $0 > 0 }
        let isOnlyNegative = row.allSatisfy { $0 < 0 }
        
        // filtering out the elements in an array that has only either negative or positive values
        
        guard isOnlyPositive || isOnlyNegative else { continue }
        
        // "abs" absolute number
        
        let isValidRow = row.allSatisfy { abs($0) >= 1 && abs($0) <= 3 }
...
let resultSafeReports = numberOfSafeReports(input: differences)

print(resultSafeReports.count)
